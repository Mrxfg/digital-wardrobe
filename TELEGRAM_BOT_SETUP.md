...
```bash
# On your VM (186.246.5.37)
cd /opt/digital-wardrobe
docker compose up -d

# Check if bot endpoint is working
curl http://186.246.5.37:8000/bot/me
```
...
```bash
curl -X POST "http://186.246.5.37:8000/bot/set-webhook" \
  -H "Content-Type: application/json" \
  -d '{"url": "http://186.246.5.37:8000/bot/webhook"}'
```

**Option B: Using Telegram API directly**

```bash
curl -X POST "https://api.telegram.org/bot8930175741:AAFn20YgCQWSDh-avVP10H2gQrJml8-9m-I/setWebhook" \
  -H "Content-Type: application/json" \
  -d '{"url": "http://186.246.5.37:8000/bot/webhook"}'
```
...
curl http://186.246.5.37:8000/bot/webhook-info

# Expected response:
{
  "ok": true,
  "result": {
    "url": "http://186.246.5.37:8000/bot/webhook",
...
```bash
# Delete webhook
curl -X POST http://186.246.5.37:8000/bot/delete-webhook
...
curl -X POST "https://api.telegram.org/bot8930175741:AAFn20YgCQWSDh-avVP10H2gQrJml8-9m-I/sendMessage" \
  -H "Content-Type: application/json" \
  -d '{
...
Telegram API
    ↓
POST http://186.246.5.37:8000/bot/webhook
    ↓
...
curl "https://api.telegram.org/bot8930175741:AAFn20YgCQWSDh-avVP10H2gQrJml8-9m-I/getWebhookInfo"
```
...
- **API Documentation:** http://186.246.5.37:8000/docs
