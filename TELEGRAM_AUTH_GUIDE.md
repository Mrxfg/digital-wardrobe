...
async function authenticateUser() {
    try {
        const response = await fetch('http://your-server-ip:8000/auth/telegram-webapp', {
            method: 'POST',
            headers: {
...
async function getMyClothes() {
    ...
    const response = await fetch('http://your-server-ip:8000/clothes', {
        headers: {
...
ALGORITHM=HS256
DATABASE_URL=postgresql://postgres:your-strong-db-password@localhost:5432/digital_wardrobe
```
