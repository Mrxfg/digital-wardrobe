...
from app.schemas.capsule_item import (
    CapsuleItemCreate,
    CapsuleItemResponse
)
...
def update_capsule(
    ...
    existing = db.query(Capsule).filter(
        Capsule.id == capsule_id,
        Capsule.user_id == current_user["user_id"]
    ).first()
    ...
    update_data = capsule.model_dump(
        exclude_unset=True
    )
...
@router.post(
    "/{capsule_id}/items",
    response_model=CapsuleItemResponse
)
def add_item_to_capsule(
    ...
    count = db.query(CapsuleItem).filter(
    ...
    existing_item = db.query(CapsuleItem).filter(
        CapsuleItem.capsule_id == capsule_id,
    ...
    if existing_item:
        raise HTTPException(
            status_code=400,
            detail="Item already exists in capsule"
        )

    capsule_item = CapsuleItem(
...
def get_capsule_items(
    ...
    items = (
        ...
        .join(
            CapsuleItem,
            ClothingItem.id == CapsuleItem.clothing_item_id
        )
        .filter(
            CapsuleItem.capsule_id == capsule_id,
...
def remove_item_from_capsule(
    ...
    if not capsule_item:
        raise HTTPException(
            status_code=404,
            detail="Item not found in capsule"
        )
